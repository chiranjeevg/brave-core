...
export function PaymentStatusView (props: Props) {
  ...
  if (estimatedPendingDays) {
    return (
      <div className='rewards-payment-pending'>
        {props.layout !== 'narrow' && <div><MoneyBagIcon /></div>}
        <div>
          {
  ...
  if (shouldShowRewardsPaymentCompleted(props.nextPaymentDate)) {
    const message = props.earningsReceived
      ? getString('rewardsPaymentCompleted')
      : getString('rewardsPaymentProcessing')

    return (
      <div className='rewards-payment-completed'>
        {props.layout !== 'narrow' && <div><PaymentCompleteIcon /></div>}
        <div>
          {
          ...
          }&nbsp;
          {
            !props.earningsReceived &&
              <div className='rewards-payment-not-arrived'>
                {
        ...
        {props.layout === 'wide' && <div><PaymentCompleteIcon /></div>}
      </div>
    )
  }
