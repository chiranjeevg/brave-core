...
class BraveShieldsAPIBrowserTest : public InProcessBrowserTest {
 public:
  ...
  std::vector<content::RenderFrameHost*> RenderFrameHostsForPrimaryPage() {
    DCHECK(active_contents());
    return CollectAllRenderFrameHosts(active_contents()->GetPrimaryPage());
  }
...
IN_PROC_BROWSER_TEST_F(BraveShieldsAPIBrowserTest, AllowScriptsOnceDataURL) {
  EXPECT_TRUE(
      NavigateToURLUntilLoadStop("a.test", "/load_js_from_origins.html"));
  EXPECT_EQ(RenderFrameHostsForPrimaryPage().size(), 4u)
  ...
  EXPECT_TRUE(WaitForLoadStop(active_contents()));
  EXPECT_EQ(RenderFrameHostsForPrimaryPage().size(), 3u)
